    ...
    Если ТиповойМодульДиадока=неопределено Тогда
           ...
           Исключение
                   ...
                   Если мРежимОтладки Тогда
                           ...
                           КаталогСМодулем = "C:\MySpace\Bases\Контур ДДПро. Водный\Модуль\";
...
Функция ПодготовитьЭлектронныйДокумент(Результат, ВидДокументаРазвернутый, ДополнительныеПараметры)
	...
	Если ВидДокументаРазвернутый.ID = "ID_Счет" Тогда
		
		структураСчета = ТиповойМодульДиадока().МетодСервера("Модуль_ИнтеграцияУниверсальный", "СформироватьПечатнуюФормуСчетаНаОплату", Документ1С);
	...
	Иначе
		...
		Если ВРЕГ(ТипКонтента) <> ВРЕГ("UtdSellerContent") Тогда
			ВызватьИсключение "Неизвестный тип контента: " + ТипКонтента;
		КонецЕсли;
		
		Если ВидДокументаРазвернутый.ID <> "ID_СчетФактура" и ВидДокументаРазвернутый.ID <> "ID_ТОРГ12" и ВидДокументаРазвернутый.ID <> "ID_Акт" Тогда
			ВызватьИсключение "Неизвестный вид документа: " + ВидДокументаНаименование;
		КонецЕсли;
		
		Если ВидДокументаРазвернутый.ID = "ID_СчетФактура" Тогда
			ВидУПДКонтента = "УПД Счф";
		Иначе
			ВидУПДКонтента = "УПД Доп";
		КонецЕсли;
			
		Допсведения = Неопределено;
		ФИОПодписанта = " ";
		InvoiceContent=	ТиповойМодульДиадока().МетодСервера("Модуль_ИнтеграцияУниверсальный","ПолучитьXmlUTDContent", Документ1С, Неопределено, Допсведения, ФИОПодписанта, ВидУПДКонтента);
...
Функция ПолучитьТекстЗапросаДляВидаПакета(ВидПакетаРазвернутый)
	...
	Если ВидПакетаРазвернутый.ID <> "ID_ПакетПродажи" и ВидПакетаРазвернутый.ID <> "ID_ПакетАренды" Тогда
		Сообщить("Неизвестный ID пакета: " + ВидПакетаРазвернутый.ID);
		Возврат Неопределено;
	КонецЕсли;
	
	Если ВидПакетаРазвернутый.ID = "ID_ПакетПродажи" Тогда
		ТекстУсловияТипОснования = "Документ.РеализацияТоваровУслуг";
	ИначеЕсли ВидПакетаРазвернутый.ID = "ID_ПакетАренды" Тогда
		ТекстУсловияТипОснования = "Документ.АР_НачислениеАренднойПлаты";
	КонецЕсли;
	...
	|	И СчетФактураВыданный.ДокументОснование ССЫЛКА " + ТекстУсловияТипОснования;
	
КонецФункции

Функция ПодготовитьПакет(ВидПакетаРазвернутый, СтрокаСписка, Пакет)
	
	ДокументСсылка = СтрокаСписка.Документ;
	Если ВидПакетаРазвернутый.ID = "ID_ПакетПродажи" Тогда
		
		РеализацияТоваровУслугСсылка = ДокументСсылка.ДокументОснование;
		
		Если РеализацияТоваровУслугСсылка.Услуги.Количество() > 0 Тогда
			ВидФормируемогоТоварногоДокумента = "ID_Акт";
		Иначе
			ВидФормируемогоТоварногоДокумента = "ID_ТОРГ12";
		КонецЕсли;
		
		ОсновнойМодуль.ЭДО_ДокументМенеджер_ПодготовитьИДобавитьДокументВПакет(Пакет, РеализацияТоваровУслугСсылка, ВидФормируемогоТоварногоДокумента);
		ОсновнойМодуль.ЭДО_ДокументМенеджер_ПодготовитьИДобавитьДокументВПакет(Пакет, ДокументСсылка, "ID_СчетФактура");
		
	ИначеЕсли ВидПакетаРазвернутый.ID = "ID_ПакетАренды" Тогда
			
		ОсновнойМодуль.ЭДО_ДокументМенеджер_ПодготовитьИДобавитьДокументВПакет(Пакет, ДокументСсылка.ДокументОснование, "ID_Счет");
		ОсновнойМодуль.ЭДО_ДокументМенеджер_ПодготовитьИДобавитьДокументВПакет(Пакет, ДокументСсылка.ДокументОснование, "ID_Акт");
		ОсновнойМодуль.ЭДО_ДокументМенеджер_ПодготовитьИДобавитьДокументВПакет(Пакет, ДокументСсылка, "ID_СчетФактура");
...
Функция ПолучитьТаблицуИспользуемыхПакетов()
	...
	ДобавитьСтрокуВТаблицуЗначений(ТЗ, "ID_ПакетПродажи",	"Пакет продажи");
	ДобавитьСтрокуВТаблицуЗначений(ТЗ, "ID_ПакетАренды",	"Пакет аренды");
...
Функция ПолучитьТаблицуИспользуемыхВидовДокументов()
	...
	ДобавитьСтрокуВТаблицуЗначений(ТЗ, "ID_ТОРГ12", 	"ТОРГ-12", 		"UtdTorg12");
	ДобавитьСтрокуВТаблицуЗначений(ТЗ, "ID_Акт", 	"Акт", 	"UtdAcceptanceCertificate");
	ДобавитьСтрокуВТаблицуЗначений(ТЗ, "ID_Счет","Счет на оплату", "Nonformalized");
...
Функция НайтиСопоставлениеДокумента(Параметры)
	...
	ДеревоКонтента = ОсновнойМодуль.XML_В_XDTO( ОсновнойМодуль.ДвоичныеДанные_В_Текст(ОсновнойМодуль.ЭДО_ПолучитьХранилищеКонтента(ДокументДД)) );
	
	Если ДокументДД.ТипДокумента = "XmlAcceptanceCertificate" или ДокументДД.ТипДокумента = "XmlTorg12" Тогда
		
		Если ДокументДД.ТипКонтента <> "UniversalTransferDocument" Тогда
			ВызватьИсключение "Формат контента не соответствует 155 формату. Сопоставление документа невозможно";
		КонецЕсли;
		
		//{ Определить параметры для поиска документа в 1С "Поступление товаров и услуг"
		НомерПоставки = ОпределитьЗначениеИнфПоляНомерПоставкиДляУПД(ДеревоКонтента);
		...
		Запрос.УстановитьПараметр("НомерДокумента",	НомерПоставки);
		...
		|	И Диадок_Документы.Документ есть NULL";
...
Функция ОпределитьЗначениеИнфПоляНомерПоставкиДляУПД(ДеревоКонтентаУПД)
	
	Попытка
		элемент = ДеревоКонтентаУПД.Документ.СвСчФакт.ИнфПолФХЖ1.ТекстИнф;
	Исключение
		ВызватьИсключение "В документе отсутствует информационное поле: ""номер поставки"" . Сопоставление документа невозможно";
	КонецПопытки;
	
	
	Если ТипЗнч(элемент) = Тип("СписокXDTO") Тогда
		
		Для Каждого элементИнфо Из элемент Цикл
			
			Если элементИнфо.Идентиф = "номер поставки" Тогда
				Возврат элементИнфо.Значен;
			КонецЕсли;
	...
	Иначе
		...
		Если элемент.Идентиф = "номер поставки" Тогда
			Возврат элемент.Значен;
		КонецЕсли;
...
Функция ПровестиАнализДокумента(Параметры)
	...
	Если ДокументДД.ТипДокумента = "XmlAcceptanceCertificate" или ДокументДД.ТипДокумента = "XmlTorg12" Тогда
		...
		Если ДокументДД.ТипКонтента <> "UniversalTransferDocument" Тогда
			ВызватьИсключение "Формат контента не соответствует 155 формату. Анализ документа невозможен";
		КонецЕсли;
		
		Если НЕ ЗначениеЗаполнено(ДокументДД.Документ) Тогда
			ВызватьИсключение "Документ не сопоставлен. Анализ документа невозможен";
		КонецЕсли;
		
		Если ДокументДД.Документ.СуммаДокумента <> ДокументДД.СуммаДокумента Тогда
			Ошибки.Добавить("Сумма электронного документа (" + Формат(ДокументДД.СуммаДокумента, "ЧДЦ=0;") + ") не соответствует фактической (" + Формат(ДокументДД.Документ.СуммаДокумента, "ЧДЦ=0;") + ")");
		КонецЕсли;
		
		Если Ошибки.Количество() > 0 Тогда
			
			Результат.Вставить("Ошибки", Ошибки);
			Результат.Вставить("ДействиеВДД", "ОтказатьВПодписи");
			
		Иначе
			
			Результат.Вставить("ДействиеВДД", "Подписать");
			
		КонецЕсли;
		
		ТипДокументаПропатченный = СтрЗаменить(ДокументДД.ТипДокумента, "Xml", "Utd");
		ТипКонтентаДляОтвета = ОсновнойМодуль.ДД_Компонента_ИмяКонтентаОтветаДляТипаДокумента(ТипДокументаПропатченный, Результат.ДействиеВДД);
		Результат.Вставить("КонтентОтвета", ПодготовитьКонтентОтвета(ДокументДД, ТипКонтентаДляОтвета, Ошибки, ДокументДД.Документ));
		
		ОсновнойМодуль.ЭДО_СправочникМенеджер_УстановитьЗначениеДопРеквизита(ДокументДД, "АнализПроведен", Истина);
		Возврат Результат;
...
Функция ПодготовитьКонтентОтвета(ДокументДД, ТипКонтентаОтвета, МассивОшибок, Документ1С)
	
	Content = НовыйКонтент(ТипКонтентаОтвета);
	
	Если нрег(Content.Тип().Имя) = "utdbuyercontent" Тогда
		...
		Content.OperationContent= "Поступление товаров";     
		Content.Creator = Документ1С.Организация.НаименованиеПолное+", " +Документ1С.Организация.ИНН+"-"+Документ1С.Организация.КПП;
...
Функция ЗаполнитьРеквизитыПодписантаПоОрганизации1С(Реквизиты, Организация)
	...
	|ИЗ
	|	РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних() КАК ОтветственныеЛицаОрганизаций
	|ГДЕ
