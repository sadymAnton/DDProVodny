...
Функция ПодготовитьПакет(ВидПакетаРазвернутый, СтрокаСписка, Пакет)
	...
	Если ВидПакетаРазвернутый.ID = "ID_ПакетПродажи" Тогда
		...
		//{ Определить документ "Реализация товаров и услуг" - основание счета-фактуры
		Если ДокументСсылка.ДокументыОснования.Количество() = 0 Тогда
			ВызватьИсключение "В документе ""Счет-фактура выданный"" не указано ни одного документа-основания. Невозможно сформировать пакет";
		КонецЕсли;
		
		РеализацияТоваровУслугСсылка = ДокументСсылка.ДокументыОснования[0].ДокументОснование;
		Если ТипЗнч(РеализацияТоваровУслугСсылка) <> Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
			ВызватьИсключение "Документ-основание в счете-фактуре в строке 1 не является документом типа ""Реализация товаров и услуг"". Невозможно сформировать пакет";
		КонецЕсли;
		//}
